...
import './About.css'; // Import CSS for navbar styling and blur effect
...
function Navbar() {
  ...
  return (
    <div className={`w-full fixed z-[999] px-20 py-8 font-['Neue Montreal'] flex justify-between items-center -mt-3 font-thin transition-all duration-300 ${visible ? '' : 'transform translate-y-[-100%]'}`}>
      <div className="logo">
      ...
      <div className="links flex gap-10">
        {["Services", "Our Works", "About Us", "Insights", "Contact"].map((item, index) => (
          <a key={index} className={`text-base capitalize  ${index === 4 ? 'ml-64' : ''}`}>
            {item}
          </a>
        ))}
      </div>
