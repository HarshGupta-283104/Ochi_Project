...
import OchiImage from '../Images/Ochi.jpg'; // Correctly import the image
      ...
      <div className="textstructure mt-36 px-20">
        {['We Create', 'Eye Opening', 'Presentation'].map((items, index) => (
          <div key={index} className="masker flex">
            {index === 1 && (
              <motion.div initial={{width: 0}} animate={{width: "8vw"}} transition={{ease: [0.76, 0, 0.24, 1], duration: 1}} className="w-[8vw] h-[vh] mr-3 rounded-md overflow-hidden relative top-[1vw]">
                <img src={OchiImage} alt="Ochi" /> {/* Use the imported image */}
              </motion.div>
            )}
